...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return jsonError('Failed to fetch admin profile', 'SERVER_ERROR', 500)
  }
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const newUsername = body.newUsername || body.username
    const currentPassword = body.currentPassword

    if (!newUsername || typeof newUsername !== 'string') {
      return jsonError('Valid username is required', 'VALIDATION_ERROR', 400)
    }
    ...
    return jsonError('Failed to update profile', 'SERVER_ERROR', 500)
  }
}
