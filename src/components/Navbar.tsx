...
export function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-30 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="fixed inset-x-0 top-[65px] bg-white border-b border-black/10 shadow-apple-modal p-6 flex flex-col gap-4 animate-slide-up max-h-[85vh] overflow-y-auto">
            <div className="flex flex-col gap-1">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  href={link.path}
                  className={`px-4 py-3 rounded-apple text-base font-medium transition-all ${
                    isActive(link.path)
                      ? 'bg-star-50 text-star-600 font-semibold'
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-col gap-2.5">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="flex items-center justify-center gap-2 py-3 rounded-apple bg-[#25D366] text-white font-semibold text-sm shadow-sm"
              >
                <MessageSquare className="w-4 h-4 fill-current" />
              ...
              <a
                href={getDialerUrl(SITE_CONFIG.phone)}
                className="flex items-center justify-center gap-2 py-3 rounded-apple bg-star-600 text-white font-semibold text-sm shadow-sm"
              >
              ...
              <Link
                href="/admin"
                className="flex items-center justify-center gap-2 py-2.5 rounded-apple border border-slate-200 text-slate-500 text-xs font-medium hover:bg-slate-50 transition-colors"
              >
