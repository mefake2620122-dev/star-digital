...
export function MobileStickyBar({ contact }: MobileStickyBarProps) {
      ...
      <div className="grid grid-cols-2 p-2 gap-2">
        ...
        >
          ...
          <div className="flex flex-col text-left leading-tight">
            <span>Call Now</span>
            <span className="text-[10px] font-normal text-white/80">{activePhone}</span>
          </div>
        </a>

        <a
          href={getNativeWhatsAppUrl(whatsappMsg, activeWhatsapp)}
          onClick={(e) => {
            e.preventDefault()
            window.location.href = getNativeWhatsAppUrl(whatsappMsg, activeWhatsapp)
          }}
